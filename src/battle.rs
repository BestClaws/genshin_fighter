...
use crate::domain::{DomainPlugin, spawn_domain_sys};
...
impl Plugin for BattlePlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_system_set(SystemSet::on_enter(BattleState::Loading).with_system(beg_load_screen_sys));
        app.add_system_set(SystemSet::on_exit(BattleState::Loading).with_system(end_load_screen_sys));
...
fn spawn_all(mut commands: Commands, windows: Res<Windows>, domain_assets: Res<DomainAssets>, image_assets: Res<Assets<Image>>) {
    info!("loading finished, spawing entities");
    spawn_domain_sys(commands, windows, domain_assets, image_assets);
}
